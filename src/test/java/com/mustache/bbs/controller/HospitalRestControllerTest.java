...
//    MockMvc mockMvc;
//
//    @MockBean // autowired 아님 --> hospitalService 테스트를 위해 가짜 객체를 쓰겠다 이말이다
//    HospitalService hospitalService;  // --> 쓰면 좋은점 , DB와 상관없이 테스트 가능
//
//    @Test
//    @DisplayName("1개의 json형태로 response가 잘 오는지") //비즈니스로직(service 검증 x) 컨트롤러만 검증한다
//    void jsonResponse() throws Exception {
...
//        int hospitalId =2321;
//
//        String url = String.format("/api/vi/hospitals/%d",hospitalId);
//
//        mockMvc.perform(get(url))
//                .andExpect(status().isOk())
//                .andExpect(jsonPath("%.hospitalName").exists())//$ 루트
//                .andExpect(jsonPath("$.hospitalName").value("노소아청소년과의원"))
//                .andDo(print());
//
//        verify(hospitalService).getHospital(hospitalId); // 이 메소드가 호출 있었는지 확인
//    }
}